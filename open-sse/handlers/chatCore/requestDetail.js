...
export function saveUsageStats({ provider, model, tokens, connectionId, apiKey, endpoint, label = "USAGE" }) {
  ...
  // Normalize to OpenAI token shape for storage. Keep cache/reasoning fields —
  // dropping them zeroed cacheHitRatio and undercounted cost (cache_creation is
  // the bulk of Claude spend). saveRequestUsage prices + aggregates these.
  const normalized = {
