...
export function logUsage(provider, usage, model = null, connectionId = null, apiKey = null) {
  ...
  // NOTE: persistence is intentionally NOT done here. The streaming flush calls
  // logUsage() AND then fires onStreamComplete() → saveUsageStats(), which is the
  // single canonical writer (same path non-streaming uses, and it carries endpoint).
  // Writing here too double-counted every streaming request in usageHistory/usageDaily.
  // logUsage is now console-only.
}
