...
export class DefaultExecutor extends BaseExecutor {
  ...
  resolveAuthDescriptor() {
    ...
    // Per-tool beta flags from typed Anthropic tools (e.g. web_search_20250305 → web-search-2025-03-05).
    // Union them into the existing anthropic-beta header without leaking the internal field.
    if (this.config?.format === "claude" && Array.isArray(body?._extraBetaFlags) && body._extraBetaFlags.length > 0) {
      const betaKey = headers["anthropic-beta"] !== undefined
        ? "anthropic-beta"
        : (headers["Anthropic-Beta"] !== undefined ? "Anthropic-Beta" : "anthropic-beta");
      const existing = new Set((headers[betaKey] || "").split(",").map(f => f.trim()).filter(Boolean));
      for (const flag of body._extraBetaFlags) existing.add(flag);
      headers[betaKey] = Array.from(existing).join(",");
    }
