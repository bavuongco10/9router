...
export class BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      const headers = this.buildHeaders(credentials, stream, transformedBody);

      if (!retryAttemptsByUrl[urlIndex]) retryAttemptsByUrl[urlIndex] = 0;

      // Baseline connect timeout: provider-specific override, else global default.
      let currentTimeout = this.config?.timeoutMs || FETCH_CONNECT_TIMEOUT_MS;
      // Boost for reasoning/thinking models — headers can be delayed for minutes.
