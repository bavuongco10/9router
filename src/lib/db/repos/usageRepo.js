...
async function getApiKeyMapCached() {
  ...
  try {
    ...
    const map = {};
    for (const k of all) if (k.key) map[k.key] = k.name;
    apiKeyCache.map = map;
  ...
  const recentRequests = recentRows
    .map((r) => {
      ...
      return {
        ...
        completionTokens: t.completion_tokens || t.output_tokens || 0,
        status: r.status || "ok",
        keyName: r.apiKey ? (apiKeyMap[r.apiKey]?.name || null) : null,
      };
    })
    .filter((e) => {
      if (e.promptTokens === 0 && e.completionTokens === 0) return false;
  ...
  if (useDailySummary) {
    ...
    for (const dr of dayRows) {
      ...
      for (const [akKey, ak] of Object.entries(day.byApiKey || {})) {
        ...
        if (!stats.byApiKey[akKey]) {
          stats.byApiKey[akKey] = { requests: 0, promptTokens: 0, completionTokens: 0, cacheReadTokens: 0, cost: 0, rawModel, provider: providerDisplayName, apiKeyMasked, keyName, apiKeyKey, lastUsed: dateKey };
        }
    ...
    for (const r of filtered) {
      ...
      if (r.apiKey && typeof r.apiKey === "string") {
        ...
        if (!stats.byApiKey[akKey]) {
          stats.byApiKey[akKey] = { requests: 0, promptTokens: 0, completionTokens: 0, cacheReadTokens: 0, cost: 0, rawModel: r.model, provider: providerDisplayName, apiKeyMasked, keyName, apiKeyKey: apiKeyMasked, lastUsed: r.timestamp };
        }
        ...
        if (!stats.byApiKey["local-no-key"]) {
          stats.byApiKey["local-no-key"] = { requests: 0, promptTokens: 0, completionTokens: 0, cacheReadTokens: 0, cost: 0, rawModel: r.model, provider: providerDisplayName, apiKeyMasked: null, keyName: "Local (No API Key)", apiKeyKey: "local-no-key", lastUsed: r.timestamp };
        }
